<script lang="ts">
	...
	console.log('Mis leads data:', data);
	...
	function render(key: string, lead: Lead) {
		switch (key) {
			...
			case 'status':
				if (lead.interest_level?.value == 'potential') return '<span class="bg-lime-600 rounded-full text-white px-7 py-2 ">Potencial</span>';
				if (lead.interest_level?.value == 'NO_ANSWER') return '<span class="bg-red-600 rounded-full text-white px-7 py-2 ">No contestar</span>';
				if (lead.interest_level?.value == 'NO_RESPONSE') return '<span class="bg-yellow-600 rounded-full text-white px-7 py-2 ">Sin respuesta</span>';
			case 'createdAt':
...
<Modal bind:open={showChangeStatusModal} size="md" autoclose={false}>
    ...
    >
        ...
        <div class="flex items-center gap-3 justify-center mb-6">
            ...
            <span class="{currentLeadStatusColor} rounded-full text-white px-6 py-1 text-sm">{currentLeadStatus}</span>
        </div>
            ...
            <div class="flex gap-3 justify-center flex-wrap">
                {#if currentLeadStatus !== 'Potencial'}
                    <button
                        ...
                        class="rounded-full px-6 py-2 text-white transition-all
                            {selectedStatus?.value === 9 ? 'bg-lime-700 ring-2 ring-lime-400' : 'bg-lime-600 hover:bg-lime-700'}"
                    >
                        Potencial
                    </button>
                {/if}
                ...
                {#if currentLeadStatus !== 'Sin respuesta'}
                    <button
                        ...
                        class="rounded-full px-6 py-2 text-white transition-all
                            {selectedStatus?.value === 11 ? 'bg-yellow-600 ring-2 ring-yellow-400' : 'bg-yellow-500 hover:bg-yellow-600'}"
                    >
                        Sin respuesta
                    </button>
                {/if}
                ...
                {#if currentLeadStatus !== 'No contestar'}
                    <button
                        ...
                        class="rounded-full px-6 py-2 text-white transition-all
                            {selectedStatus?.value === 10 ? 'bg-red-600 ring-2 ring-red-400' : 'bg-red-500 hover:bg-red-600'}"
                    >
                        No contestar
                    </button>
                {/if}
...
<Modal bind:open={showBudgetModal} size="md" autoclose={false}>
    ...
    >
        ...
        <!-- Seleccionar archivo -->
        <div>
            <label for="file-input" class="block border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-gray-400 transition">
