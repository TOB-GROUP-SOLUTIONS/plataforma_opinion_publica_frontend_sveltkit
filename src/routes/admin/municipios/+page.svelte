<script lang="ts">
    ...
    function openEditModal(municipio: any) {
        isEditing = true;
        formData = { ...municipio, id: municipio.id.toString() };
        showModal = true;
    }
    ...
    async function deleteMunicipio(id: string) {
...
<div class="h-full w-full bg-[#F8FAFC] -mx-2 lg:-mx-4 -mt-4 -mb-36 md:-mb-8">
					...
					<tr class="hover:bg-[#F8FAFC] transition-colors group">
						...
						<td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button type="button" on:click={() => deleteMunicipio(entidad.id.toString())} class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors" title="Eliminar">
	...
	<Modal bind:open={showModal} size="md" outsideclose autoclose={false} class="w-full">
		<form method="POST" action={isEditing ? '?/update' : '?/create'} use:enhance={handleFormEnhanced}>
            ...
            <div class="space-y-4">
                <div>
                    <Label for="entidad" class="mb-2">Nombre del Municipio</Label>
                    <Input id="entidad" name="entidad" bind:value={formData.entidad} required placeholder="Ej: La Paz, Santa Cruz..." />
                </div>
                
                <div>
                    <Label for="tipo" class="mb-2">Tipo de Municipio</Label>
                    <Input id="tipo" name="tipo" bind:value={formData.tipo} required placeholder="Ej: CAPITAL, PROVINCIA, RURAL..." />
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Usa texto clave que define el espectro de búsqueda de este municipio.</p>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 mt-6 border-t pt-4">
                ...
                <Button type="submit" color="blue" disabled={isSubmitting}>
                    {isSubmitting ? 'Guardando...' : 'Guardar Municipio'}
                </Button>
            </div>
        </form>
	</Modal>
